...
export default function Account() {
  ...
  const [hotelSubmissions, setHotelSubmissions] = useState([]);
    ...
    const fetchUserDetails = async () => {
      try {
        ...
        const submissionResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/thirdpartyequipment`,
          { withCredentials: true }
        );
  ...
  const navigateToForm = () => {
    router.push("/accounts/displaytblthird");
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="mt-10 p-4 relative">
        {userRole === 1 && (
          <div>
            <button
              onClick={navigateToForm}
              className="absolute top-0 right-0 bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600"
            >
              Form
            </button>
            <h2 className="text-2xl font-bold mb-4">Third Party Equipment Form</h2>
                ...
                {hotelSubmissions.length === 0 ? (
                  ...
                  hotelSubmissions.map((submission) => (
                    <tr key={submission.id} className="hover:bg-gray-100">
                      ...
                      <td className="border border-gray-300 px-4 py-2">{submission.propertyName}</td>
                      <td className="border border-gray-300 px-4 py-2">
                        {submission.isVerified ? "Verified" : "Not Verified"}
                      </td>
                      <td className="border border-gray-300 px-4 py-2 space-x-2">
                        <Link
                          href={`/accounts/form/${submission.CreatedBy}`}
